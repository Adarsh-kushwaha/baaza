...
/** Resting search field on the tab; tapping it raises a full-screen overlay over the tab bar. */
export function SearchScreen() {
  ...
  return (
    <div className="px-4 pt-[calc(env(safe-area-inset-top)+16px)]">
      ...
      <CategoryChips onPick={openWith} />

      {open && <SearchOverlay query={query} setQuery={setQuery} onClose={closeOverlay} onPick={openWith} />}
    </div>
  );
}
...
function SearchOverlay({ query, setQuery, onClose, onPick }: SearchOverlayProps) {
      ...
      <div className="flex-1 overflow-y-auto overscroll-contain pb-[calc(env(safe-area-inset-bottom)+24px)]">
        {!hasQuery ? (
          <div className="px-4">
            ...
            <CategoryChips onPick={onPick} />
          </div>
        ) : results.length > 0 ? (
          <ul aria-label={`Results for ${query}`}>
            {results.map((card) => (
              <PlaylistRow key={card.slug} card={card} onOpen={() => addRecentSearch(query)} />
            ))}
          </ul>
        ) : (
...
function CategoryChips({ onPick }: { onPick: (category: string) => void }) {
  return (
